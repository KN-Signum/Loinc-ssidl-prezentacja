...
import { Droplets, FlaskConical, Syringe, TestTube2, Truck } from "lucide-react";
...
import {
  MaterialInfo,
  TestedMaterial,
} from "../../../features/specimenDefinition/SpecimenDefinition";
import { DetailSection, SectionTitle } from "../primitives";
import { SpecimenCommentSection } from "./SpecimenCommentSection";

export const MaterialSection: React.FC<{
  typeCollected: MaterialInfo | null;
  testedMaterials: TestedMaterial[];
  comment: string | null;
}> = ({ typeCollected, testedMaterials, comment }) => {
  if (!typeCollected && testedMaterials.length === 0 && !comment) return null;
  ...
  return (
    <DetailSection variant="muted">
      ...
      {testedMaterials.length > 0 && (
        <div
          className={
            typeCollected ? "mt-4 border-t border-slate-200 pt-4" : ""
          }
        >
      ...
      <SpecimenCommentSection comment={comment} />
    </DetailSection>
  );
};
